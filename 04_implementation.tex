...
Jetstream pruža izbor između korištenja \textbf{Livewire} ili Inertia.js \textit{frontend scaffoldinga}. O tom izboru ovisi i odabrani jezik za predloške (engl. \textit{templating language}) jer uz Livewire to je \textbf{Blade}, a uz Inertia.js to je Vue.js~\cite{jetstreamIntro}.

Za instaliranje Jetstreama s Livewire \textit{frontend scaffoldingom} i to s uključenom podrškom za timove koristi se naredba \texttt{php artisan jetstream:install livewire -{}-teams}~\cite{jetstreamInstallation}.
...
U poglavlju~\ref{subsection:dbInteraction} bit će više rečeno o načinima na koje se ostvaruje interakcija s bazom podataka te o samim modelima i relacijama.
...
U klasi modela definiraju se njegovi atributi (\textit{fillable} - oni koji su \textit{mass assignable} tj. dozvoljeno je da ih dodijele korisnici aplikacije te \textit{hidden} - oni koji su skriveni pri serijalizaciji), relacije (o kojima će biti više riječi u poglavlju~\ref{paragraph:relations}), \textit{traits} tj. osobine koje koristi, događaje koje emitira itd.

U ispisu~\ref{modelUser} prikazan je dio k\^oda iz klase modela \texttt{User} gdje je vidljivo kako koristi nekolicinu osobina (engl. \textit{traits}) te definiranje atributa modela.
...
Također, tvornicama modela mogu se definirati stanja (engl. \textit{factory states}) koja omogućavaju primjenu diskretnih izmjena tj. modifikaciju odabranih atributa na nekoj tvornici modela. U ispisu~\ref{factoryStatesUser} prikazan je dio k\^oda iz tvornice modela \texttt{UserFactory} gdje se definira stanje tvornice modela za stvaranje administrator korisnika. Vidljivo je kako to stanje obavlja izmjene samo na odabranim atributima, a ostali će atributi biti postavljeni kao u izvornoj definiciji tvornice modela~\cite{factories}.
...
Model \texttt{Team} definira veze prema modelu \texttt{Project}, kao što je vidljivo u ispisu~\ref{relationsTeam}, a nasljeđujući model \texttt{JetstreamTeam} definirane su veze prema modelima \texttt{User}, \linebreak \texttt{Membership} i \texttt{TeamInvitation} te su te metode prikazane u ispisu~\ref{relationsJetstreamTeam}.
...
Model \texttt{Project} definira veze prema modelima \texttt{Team}, \texttt{User} i \texttt{Post} vidljivo u ispisu~\ref{relationsProject}, te implementirajući \textit{interface} \texttt{HasMedia} i koristeći osobinu \linebreak \texttt{InteractsWithMedia} definira i vezu prema modelu \texttt{Resource} vidljivo u ispisu~\ref{relationsProjectTrait}.
...
Model \texttt{Post} definira veze prema modelima \texttt{Project}, \texttt{User} i \texttt{Comment} kao što je prikazano u ispisu~\ref{relationsPost}.
...
Model \texttt{Comment} definira veze prema modelima \texttt{Post} i \texttt{User} kao što je prikazano u ispisu~\ref{relationsComment}.
...
Izvršavanjem Artisan naredbe \texttt{route:list} mogu se izlistati sve rute definirane u aplikaciji te je rezultat izvršavanja prikazan na slici~\ref{fig:routes}.
\begin{figure}[H]
	\includegraphics[width=0.85\linewidth,clip=]{assets/routes.png}
	\centering
	\caption{Lista svih ruta definiranih u aplikaciji pomoću  Artisan naredbe \texttt{route:list}}
	\label{fig:routes}
\end{figure}
...
\begin{figure}[H]
	\includegraphics[width=0.8\linewidth,clip=]{assets/register.png}
	\centering
...
\begin{figure}[H]
	\includegraphics[width=0.8\linewidth,clip=]{assets/login.png}
	\centering
	\caption{Prijava}
	\label{fig:login}
\end{figure}
...
\subsubsection{Vrata}
Vrata (engl. \textit{gates}) određuju ima li korisnik ovlasti za izvršavanje određene radnje. Obično se definiraju unutar \texttt{boot} metode u klasi {\texttt{App\textbackslash Providers\textbackslash \linebreak 
AuthServiceProvider}} koristeći fasadu \texttt{Gate}~\cite{authorization}.

U ispisu~\ref{gatesAdmin} prikazana je \texttt{boot} metoda u kojoj se definiraju vrata \linebreak 
\texttt{'admin-privileges'} te se koristeći metodu \texttt{before} definira \textit{closure} koji će biti provjeravan prije svih ostalih autorizacijskih provjera.
...
Kao primjer politike u ispisu~\ref{postPolicy} prikazana je datoteka \texttt{PostPolicy.php} tj. politika koja definira autorizaciju akcija nad modelom \texttt{Post}. Tu je definirano kako pojedinu objavu mogu vidjeti samo članovi tima kojem objava pripada, također kako objavu može urediti i ažurirati samo onaj korisnik koji ju je i objavio, a ovlasti za izbrisati objavu imaju vlasnik tima, vlasnik/tvorac projekta kojem objava pripada te autor objave.

\begin{lstlisting}[caption={Dio k\^oda politike za autorizaciju akcija nad modelom \texttt{Post}}, label=postPolicy]
<?php
...
Livewire je \textit{full-stack} razvojni okvir za Laravel koji olakšava razvijanje dinamičkog korisničkog sučelja. Razvio ga je Caleb Porzio, otvorenog je k\^oda te je k\^od dostupan na platformi GitHub~\cite{livewireGithub}. Radi na način da se prvo inicijalno prikazuje Livewire komponenta na stranici, a zatim kada korisnik s njom interaktira Livewire šalje AJAX zahtjev prema poslužitelju s ažuriranim podatcima. Poslužitelj zatim iznova prikazuje tu komponentu i vraća novi HTML, a Livewire mutira DOM (engl. \textit{Document Object Model}) tako da se učinjene promjene očitaju na stranici~\cite{livewire}.

Livewire komponenta kreira se pozivom Artisan naredbe \texttt{make:livewire} te će se tada kreirati sljedeće dvije datoteke - u direktoriju \texttt{app/Http/Livewire} kreirat će se klasa Livewire komponente, a u direktoriju \texttt{resources/views/livewire} \linebreak \texttt{.blade.php} datoteka Livewire komponente. U ispisu~\ref{showPostClass} prikazana je klasa Livewire komponente \texttt{ShowPost}.
...
Livewire komponenta može biti uključena u Blade predložak koristeći direktivu \linebreak \texttt{@livewire}.

Validacija atributa komponente u Livewireu obavlja se pozivom metode \linebreak \texttt{\$this->validate()} koja zatim provjerava pravila definirana unutar podatkovnog člana \texttt{\$rules} za pojedini atribut~\cite{livewireValidation}.

Kako bi se unutar Livewire komponente koristila paginacija potrebno je koristiti osobinu (engl. \textit{trait}) \texttt{WithPagination}~\cite{livewirePagination}.
...
Tako, gledajući prethodni ispis, metode \texttt{edit}, \texttt{update}, \texttt{delete}, \texttt{destroy} u klasi Livewire komponente izvršavaju se nakon interakcije korisnika nad određenim elementom na stranici - primjerice korisnik interaktira s botunom (engl. \textit{button}) koji "osluškuje" određeni događaj putem direktive \texttt{wire} kao npr. \texttt{wire:click="edit"}, \linebreak \texttt{wire:click="update"},  \texttt{wire:click="delete"}, \texttt{wire:click="destroy"} u pripadajućem Blade predlošku.

Kako bi se akcije u Livewireu mogle autorizirati potrebno je koristiti osobinu (engl. \textit{trait}) \texttt{AuthorizesRequests} te je zatim moguće pozvati metodu \texttt{\$this->authorize()} unutar Livewire komponente~\cite{livewireAuthorization}. 
...
\begin{figure}[H]
	\includegraphics[width=0.85\linewidth,clip=]{assets/minio-login.png}
	\centering
	\caption{MinIO Console - MinIO Object Store zaslon za prijavu (engl. \textit{login})}
	\label{fig:minioLogin}
\end{figure}

\begin{figure}[H]
	\includegraphics[width=0.85\linewidth,clip=]{assets/minio-object-browser.png}
	\centering
	\caption{MinIO Console - MinIO Object Browser zaslon i stvoreni \textit{bucket}}
	\label{fig:minioObjectBrowser}
\end{figure}

\subsection{Implementacija timskih resursa koristeći Spatie Laravel Media Library paket}
Za implementaciju timskih resursa korišten je Laravel Media Library \linebreak (\texttt{spatie/laravel-medialibrary}) paket razvijen od strane Spatie tima - belgijske \textit{development} agencije koja razvija mnogo poznatih paketa otvorenog k\^oda za PHP i Laravel ekosustav.
...
Za ostvariti željenu vezu između modela \texttt{Project} i \texttt{Resource} potrebno je da model \texttt{Project} implementira \textit{interface} \texttt{HasMedia} i osobinu (engl. \textit{trait}) \linebreak \texttt{InteractsWithMedia}, a model \texttt{Resource} nasljeđuje bazni model \texttt{Media} iz Media Library paketa. Također je potrebno u konfiguracijskoj datoteci \texttt{config/ \linebreak media-library.php} navesti naziv klase \textit{custom} medijskog modela kao u ispisu~\ref{mediaModelConfig}.
...
Zadano Laravel aplikacija ne sadrži direktorij \texttt{lang} u kojem bi bile pohranjene potrebne jezične datoteke, ali ga se može stvoriti izvođenjem Artisan naredbe \linebreak \texttt{lang:publish}~\cite{localization}. 
...
U konfiguracijsku datoteku za Nginx poslužitelj \texttt{default.conf} dodana je linija \linebreak \texttt{rewrite \textasciicircum /(en|hr)/?(.*)?  https://teamstructor.test/\$2?locale \linebreak =\$1 permanent;} te u \texttt{.htaccess} datoteku linija \texttt{RedirectMatch \linebreak "\textasciicircum /(en|hr)/?(.*)?" "/\$2?locale=\$1"} što omogućava da se u web pregledniku URL unesen u formi \url{https://teamstructor.test/hr} tumači kao \linebreak \url{https://teamstructor.test/?locale=hr}.

Naposlijetku je implementiran i \textit{language switcher} tj. padajući izbornik (engl. \textit{dropdown menu}) za promjenu jezika prikazan na slici~\ref{fig:localeSwitcher}.
\begin{figure}[H]
	\includegraphics[width=0.9\linewidth,clip=]{assets/language-switcher.png}
	\centering
	\caption{Padajući izbornik u navigacijskoj traci koji služi za promjenu trenutnog jezika aplikacije}
...
Također je korišten i \textit{code style fixer} \textbf{Laravel Pint}~\cite{pint}, izgrađen na PHP-CS-Fixeru, koji je automatski dostupan za korištenje uz Laravel instalaciju. Pomaže da k\^od ostane "čist" i konzistentan te se popravak \textit{code stylea} inicira izvođenjem naredbe \linebreak \texttt{.vendor/bin/pint} iz terminala. 
...
U aplikacijskom \texttt{test} direktoriju zadano se nalaze dva poddirektorija - \texttt{Feature} i \texttt{Unit}. Namjena \textit{unit} testova jest testiranje manjih izoliranih dijelova k\^oda, a namjena testova funkcionalnosti jest testiranje većih dijelova k\^oda tj. pojedinih funkcionalnosti u cijelosti~\cite{testing}.

Kako se testiranje ne bi provodilo nad glavnom "radnom" bazom podataka i tako se obavljale promjene nad stvarnim zapisima u bazi, za potrebe testiranja definirano je posebno izolirano \texttt{sqlite\_testing} okruženje baze podataka u memoriji (prikazano u ispisu~\ref{sqliteTesting}).
...
\textbf{Pest} PHP razvojno okruženje za testiranje potrebno je instalirati putem Composera: \texttt{composer require pestphp/pest -{}-dev -{}-with-all-dependencies}, \linebreak a zatim inicijalizirati Pest u trenutnom projektu koristeći naredbu \linebreak \texttt{./vendor/bin/pest -{}-init}~\cite{pestInstallation}.

Moguće je instalirati Pest dodatke (engl. \textit{plugins}) za Laravel i Livewire putem Composera: \texttt{composer require pestphp/pest-plugin-laravel -{}-dev} i \linebreak \texttt{composer require pestphp/pest-plugin-livewire -{}-dev}~\cite{pestPlugins}.
...
Prilikom izrade ovog završnog rada i razvijanja aplikacije koristeći se početnim kompletom Laravel Jetstream uočila sam stilsku nekonzistentnost u korisničkom sučelju pri prikazivanju profilne fotografije korisnika u navedenom Laravel \textit{first-party} paketu te sam zatim dala svoj doprinos. Bilo je potrebno \textit{forkati} izvorni repozitorij, učiniti potrebne izmjene u k\^odu na posebnoj grani te otvoriti \textit{pull request} (dostupan na poveznici \url{https://github.com/laravel/jetstream/pull/1304}). Izmjene koje sam učinila su prihvaćene od strane Laravel tima te objavljene u sklopu Laravel Jetstream izdanja v3.1.3 koje je dostupno na poveznici \url{https://github.com/laravel/jetstream/releases/tag/v3.1.3} te je  na slici~\ref{fig:jetstreamCommits} prikazana povijest \textit{commita} za to izdanje.
\begin{figure}[H]
	\includegraphics[width=1\linewidth,clip=]{assets/jetstream-commits-v3.1.3.png}
	\centering
	\caption{Povijest \textit{commita} - \texttt{laravel/jetstream v.3.1.3}}
	\label{fig:jetstreamCommits}
\end{figure}